...
# ==========================================
# 7. CMS LIBRE: HTML Y CATÁLOGO DE IMÁGENES
# ==========================================

class PaginaInicioHTML(models.Model):
    codigo_html = models.TextField(
        verbose_name="Código HTML Completo", 
        help_text="Pega aquí todo tu código. Usa las rutas del catálogo de imágenes para tus etiquetas <img>."
    )
    ...
    def __str__(self):
        return "Haz clic aquí para editar el código HTML de Inicio"


# Creamos una ruta directa y forzada para este catálogo sin tocar settings
almacenamiento_local = FileSystemStorage(location=settings.MEDIA_ROOT, base_url=settings.MEDIA_URL)

class CatalogoImagen(models.Model):
    nombre = models.CharField(max_length=100, help_text="Ej: Logo principal, Foto doctores, etc.")
    # Le inyectamos el 'storage' forzado para que no busque el 'default'
    imagen = models.ImageField(upload_to='catalogo/', storage=almacenamiento_local)
